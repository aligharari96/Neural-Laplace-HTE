...
class CRNEncoder(CRN):
    ...
    def forward(self, batch, detach_treatment=False):
        ...
        bs, seq, _ = prev_treatments.shape
        prev_times = torch.arange(1, seq+1).repeat(bs, 1).unsqueeze(2).to(prev_treatments)
        ...
        #vitals_or_prev_outputs.append(prev_times)
        ...
        outcome_pred = self.br_treatment_outcome_head.build_outcome(br, curr_treatments)#, tp_to_predict=(prev_times + 1.0).to(prev_times))
