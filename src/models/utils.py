...
class LaplaceRepresentationFunc(nn.Module):
  """
  Neural network module for Laplace representation function.
  Args:
...
class BRTreatmentOutcomeHead(nn.Module):
    ...
    def __init__(self, seq_hidden_units, br_size, fc_hidden_units, dim_treatments, dim_outcome, alpha=0.0, update_alpha=True,
        ...
        self.linear3_ = nn.Linear(self.dim_treatments, self.dim_treatments)
        ...
        self.laplace_rep_func = LaplaceRepresentationFunc(s_dim=33, out_dim=self.dim_outcome, latent_dim=self.fc_hidden_units, hidden_units=self.fc_hidden_units)
    ...
    def build_treatment(self, br, detached=False):
        ...
        treatment = self.elu3(self.linear3(br)) # Softmax is encapsulated into F.cross_entropy()
        return treatment
    ...
    def build_outcome(self, br, current_treatment):
        ...
        bs, seq, d = x.shape
        x = x.reshape(-1, d)
        #print(x.shape, tp_to_predict.reshape(-1,1).shape)
        #print(tp_to_predict.reshape(-1,1).min())
        ...
        # print(x.dtype)
        # print("tp: ", tp_to_predict.dtype)
        outcome = laplace_reconstruct(self.laplace_rep_func,
                                        x,
                                        (torch.ones(x.shape[0],1)).to(x),
                                        recon_dim=self.dim_outcome)
        
        
        outcome = outcome.reshape(bs,seq, -1)
        return outcome
